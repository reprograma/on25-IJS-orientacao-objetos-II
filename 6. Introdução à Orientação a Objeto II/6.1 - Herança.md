...
Enquanto a herança trabalha com o conceito do que a entidade é, a composição acompanha a declaração "X tem Z", e não faz sentido existir um sem o outro.
...
Nesse exemplo, existem duas classes distintas. A primeira é a classe `Carro` e a segunda é a classe `Motor`. Imagine que toda classe Carro tenha uma classe Motor, neste exemplo há uma relação entre Carro e Motor de composição, pois o motor está contido na classe Carro. Um Carro tem um Motor.
...
Neste exemplo, `Livro` e `Autor` são duas classes distintas, onde cada objeto da classe Livro possui um objeto da classe Autor. Neste caso, existirá uma relação de composição, pois a classe Autor faz parte da classe Livro. Um Livro tem um Autor.
